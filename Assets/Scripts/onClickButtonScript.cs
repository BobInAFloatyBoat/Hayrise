...
public class onClickButtonScript : MonoBehaviour
{
	...
	void Start()
	{
		if (setColors == true){
			...
			currentColor = MeshRenderer.materials[1];
		}
	}
	...
	void OnMouseOver()
    {
		...
		if (hoverColor != null){
			MeshRenderer.materials = new Material[2] {MeshRenderer.materials[0], hoverColor};
		}
		noOver = true;
    }    
    void OnMouseExit()
    {
		if (noOver == true){
			...
			if (currentColor != null){
				MeshRenderer.materials = new Material[2] {MeshRenderer.materials[0], currentColor};
			}
			noOver = false;
		}
    }
